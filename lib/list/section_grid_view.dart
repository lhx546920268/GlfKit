...
import 'package:GlfKit/list/section_sliver.dart';
import 'package:GlfKit/list/section_sliver_multi_box_adapter.dart';
...
class SectionSliverGrid extends SectionSliverMultiBoxAdaptorWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, SectionRenderSliverGrid renderObject) {
...
class SectionRenderSliverGrid extends SectionRenderSliverMultiBoxAdaptor {
  ...
  }) : _adapter = adapter, super(childManager: childManager);
  ...
  void performLayout() {
    ...
    BoxConstraints childConstraint = BoxConstraints(minWidth: 0, maxWidth: constraints.crossAxisExtent);
    ...
    //需要布局的大小
    final double pageSize = numberOfCachePages * constraints.viewportMainAxisExtent;
    ...
    if(_pageGridGeometries.isEmpty){
      //刚开始布局，肯定是从0开始
      pageGridGeometry = PageGridGeometry(endScrollOffset: pageSize, sectionInfo: _adapter.sectionInfoForPosition(0));
      _pageGridGeometries[0] = pageGridGeometry;
    }else{
      //获取距离当前页最小的，如果已经存在布局数据，就不应该从第一个item开始布局了
      int currentPageIndex = pageIndex + 1;
      pageGridGeometry = _pageGridGeometries[_pageGridGeometries.lastKeyBefore(currentPageIndex)];
    ...
    while(index > pageGridGeometry.firstChildIndex) {
      index --;
      RenderBox child = insertAndLayoutLeadingChild(childConstraint, parentUsesSize: true);
      if (child != null) {
        _children.add(child);
      } else {
        break;
    ...
    GridSectionInfo sectionInfo = _adapter.sectionInfoForPosition(indexOf(_children.last));
    //设置子视图位置
    ItemGridGeometry setupUpGeometry(RenderBox child){
      ...
      if(geometry == null){
        geometry = ItemGridGeometry(crossAxisExtent: paintCrossExtentOf(child), mainAxisExtent: paintExtentOf(child));
        pageGridGeometry.setupScrollOffset(geometry, constraints, sectionInfo, index);
      ...
      //拿可见范围内的第一个和最后一个子视图
      if(geometry.mainEnd > scrollOffset && geometry.scrollOffset < targetEndScrollOffset){

        if(firstVisibleChild == null){
          firstVisibleChild = child;
        }else if(childScrollOffset(firstVisibleChild) > geometry.scrollOffset) {
          firstVisibleChild = child;
        }
        if(lastVisibleChild == null){
          lastVisibleChild = child;
        }else if(childScrollOffset(lastVisibleChild) + paintExtentOf(lastVisibleChild) <= geometry.mainEnd){
          lastVisibleChild = child;
        }
      }

      if(currentSectionInfo == null && geometry.mainEnd >= constraints.scrollOffset){
    ...
    while(endScrollOffset < targetEndScrollOffset){
      ...
      index ++;
      RenderBox child = insertAndLayoutChild(childConstraint, after: currentLastChild, parentUsesSize: true);
      if(child != null){
        currentLastChild = child;
        endScrollOffset = math.max(setupUpGeometry(child).mainEnd, endScrollOffset);
      }else{
        reachEnd = true;
        break;
      }
    ...
    double trailingScrollOffset = childScrollOffset(lastVisibleChild) + paintExtentOf(lastVisibleChild);
    ...
    if(currentSectionInfo != null){
      ...
      if(currentSectionInfo.isHeaderStick && currentSectionInfo.isExistHeader){
        ...
        if(geometry.scrollOffset < constraints.scrollOffset){
          ...
          RenderBox child = addAndLayoutChild(childConstraint, index: index, after: lastVisibleChild);

          SectionSliverMultiBoxAdaptorParentData parentData = child.parentData as SectionSliverMultiBoxAdaptorParentData;
          parentData.layoutOffset = math.min(constraints.scrollOffset, currentSectionInfo.mainEnd - geometry.mainAxisExtent);
          parentData.crossAxisOffset = geometry.crossAxisOffset;
